...
A: 根因是**客户端某条 TLS 路径不信任代理 CA**（非代理/TLS 协议问题）。Claude Code（Bun 运行时）长对话触发的后台辅助请求使用独立于主 fetch 的 TLS 路径，不稳定地读取 `NODE_EXTRA_CA_CERTS`；校验代理自签名证书失败后发明文 `unknown_ca` alert，代理用 handshake key 解这条明文 alert 必然 AEAD 失败、误报为 `bad record MAC`。Linux 二进制 bootstrap 会自动安装并验证 MCC CA 已进入完整系统 CA bundle，同时持久化 `NODE_EXTRA_CA_CERTS` 和指向该 bundle 的 `SSL_CERT_FILE`；完成后需完全退出并重新启动 Claude Code/Orca。Linux Docker 无法修改宿主机 profile，需在宿主机运行 `setup-host.sh trust`，并在启动 Claude Code/Orca 的宿主 shell 中设置 `SSL_CERT_FILE=/etc/ssl/certs/ca-certificates.crt`。macOS/Windows 暂不自动设置该变量。`SSL_CERT_FILE` **必须指向完整系统 bundle，不能指向单个 `data/ca.crt`**，否则部分 TLS 实现会只信任 MCC CA，破坏对公网证书的信任。
