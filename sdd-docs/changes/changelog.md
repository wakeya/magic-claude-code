...
### Fixed
- **主动清洗保留 tool_reference，修复 kimi deferred 工具调用失效**：`strip_unknown_content_blocks` 开启时，主动清洗原先剥离 `tool_reference`（Claude Code 2.1.x deferred 工具加载标记），导致 kimi-k3 等 deferred MCP 工具调用退化为固定占位 `Bash{command:"true",description:"noop"}`、`thinking` 为空、陷入死循环。2026-07-21 受控实测表明 kimi 三个 Anthropic 兼容端点（moonshot k2.6、coding k2.7、coding k3）现已在 `tool_reference` 引用已定义工具时**接受**该类型；最初触发清洗的 `"unsupported content type"` 400（research/2026-05）已不复现，残留 400 真因是 `tool_reference` 指向 `tools` 中未定义的工具。`filterContentBlocks` 参数化为 `(preserveToolReference)`：主动清洗保留 `tool_reference`（恢复模型上下文），反应式 `tryRectify` 仍清除它（兜底引用未定义工具的 400）。
