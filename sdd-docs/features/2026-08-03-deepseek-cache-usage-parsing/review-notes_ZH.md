...
1. **已修复的低风险逻辑问题——负零绕过校验。** `usageNumber` 原先只判断 `number < 0`，但 IEEE-754 中 `-0.0 < 0` 为 false。commit `be8ed50` 已增加 `math.Signbit(number)` 校验和运行时负零回归测试，因此 `cached_tokens: -0.0` 不再阻止回退到 `prompt_cache_hit_tokens`。
...
该分支在声明的 DeepSeek/OpenAI usage 解析范围内逻辑正确，未发现可利用的安全漏洞。负零统计健壮性问题已在 `be8ed50` 修复；剩余的供应商 Responses 语义差异属于信息性风险，不构成安全阻断。
