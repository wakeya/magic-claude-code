...
| 序号 | 状态 | 任务 | 产出 | 验证 |
| --- | --- | --- | --- | --- |
| 1 | 已规划 | Provider 限流配置模型 | Provider 字段、SQLite schema、管理 API | 配置 round-trip 测试 |
| 2 | 已规划 | Provider 并发池与队列 | `internal/proxy/ratelimit` 或同等内部组件 | 并发、排队、超时单元测试 |
| 3 | 已规划 | 代理请求路径接线 | ServeHTTP 上游请求前后 acquire/release | 流式请求 release 测试 |
| 4 | 已规划 | 429 退避重试 | 1302/1305 识别、Retry-After、指数退避 | mock 上游 429 重试测试 |
| 5 | 已规划 | 管理后台配置 UI | ProviderModal、useApi、i18n | 前端构建与交互检查 |
| 6 | 已规划 | 观测与安全验证 | 请求日志、队列拒绝错误、测试记录 | `go test ./...`、前端构建 |
...
- [ ] 新建 Provider 能保存限流字段。
- [ ] 编辑 Provider 能回显限流字段。
- [ ] 默认只显示折叠入口或主开关，不直接展示所有高级数字字段。
- [ ] 勾选“启用请求队列”后才显示队列数字字段。
- [ ] 勾选“启用 429 退避重试”后才显示重试数字字段。
- [ ] 非法配置被前端或后端阻止。
- [ ] 前端构建通过。
