...
The project is a Go single-binary transparent proxy (`mcc`) deployed either bare-metal or in Docker. Releases are built by GitHub Actions (`.github/workflows/release.yml`), GitLab CI (`.gitlab-ci.yml`), and GitCode CI (`.gitcode/workflows/release.yml`), producing platform-specific archives:
...
Response includes `tag_name`, `html_url`, and an `assets` array with each asset's `name` and `browser_download_url`. The API is well-documented, stable, and returns JSON. Unauthenticated requests are rate-limited to 60/hour, which is sufficient for frontend checks throttled to once per 24 hours per browser plus on-demand apply checks.
...
Instead of a separate connectivity probe, the updater tries sources sequentially: GitHub first, GitCode on failure. Each source request has a 30-second timeout (inherited from the HTTP client). If the first source times out or returns a network error, the next source is tried automatically. Frontend page-load checks are throttled locally to once per 24 hours per browser.

### Binary Self-Update Constraints

| Platform | Can overwrite running binary? | Notes |
| --- | --- | --- |
| Linux | Yes | temporary file + backup + rename |
| macOS | Yes | Same as Linux |
| Windows | Yes | temporary file + backup + rename; no auto-restart |
| Docker | Check only | Container filesystem is ephemeral; show update notifications but guide users to image updates |
...
4. Docker detection (`/.dockerenv`) must keep update checks available but disable in-place self-update, guiding users to update the container image instead.
...
6. Restart after update: Linux/macOS auto-restarts via `syscall.Exec`; Windows requires manual restart (binary is replaced on disk but the running process stays on the old binary until restarted).
...
| 6 | Planned | Updater wiring and Docker detection | `cmd/server/main.go` wiring | Build; verify Docker guidance message |
...
3. A new `internal/updater` package provides:
   ...
   - `CheckForUpdate` that queries sources sequentially and returns `UpdateInfo` (current vs. latest version, download URL, release page URL, and any source-specific download headers).
...
4. Admin API endpoints:
   ...
   - `POST /api/update/apply` — downloads and applies the update; returns success, error, and an explicit `restarting` boolean indicating whether the service is restarting automatically.
...
6. On startup, the server only wires the updater for admin API use; it must not perform its own automatic update check.
7. Docker environments (`/.dockerenv` exists) keep `/api/update/check` available, disable `/api/update/apply`, and log a guidance message.
...
cmd/
  server/
    main.go             (modify: wire updater, Docker detection)
dist/
...
3. Docker environments (`/.dockerenv` exists) must instantiate the updater for update checks, but `POST /api/update/apply` must return a clear business error telling users to update the container image.
4. Windows self-update is supported: the updater writes a complete temporary binary, renames the running `.exe` to a backup, then renames the temporary binary into place. Auto-restart is not available on Windows (`syscall.Exec` returns ENOSYS); users must restart manually.
5. The updater must not run as a background polling loop or startup auto-check. Checks are triggered by authenticated admin API calls, with frontend page-load checks throttled to once per 24 hours per browser.
6. The HTTP client used for release checks must have a timeout (30 seconds) to avoid blocking the admin server.
7. Version comparison uses semantic-version parsing; a `dev` version is always considered older than any valid release tag.
8. Server startup must not make outbound update-check requests.
...
10. Archive downloads are capped at 200 MB; `SHA256SUMS.txt` downloads are capped at 1 MB; both must fail with a clear size-limit error when exceeded.
11. The frontend update notification must be best-effort — a failed check must not show an error badge or block normal usage.
...
8. Network is completely unreachable — both sources fail; the check endpoint returns a clear error; the frontend records the check attempt and does not retry page-load checks for 24 hours.
9. The admin server has no updater configured — update endpoints return HTTP 503. Docker environments still configure the updater for checks and only disable apply.
...
1. ~~Do not implement automatic process restart after binary replacement~~ — **Updated 2026-06-13**: Auto-restart implemented for Linux/macOS via `syscall.Exec` per user request. Windows requires manual restart.
2. Do not implement a background polling loop for update checks.
3. ~~Do not implement Windows self-update in this phase~~ — **Updated 2026-06-13**: Windows self-update implemented per user request (binary replacement via `os.Rename`; no auto-restart).
...
6. ~~Do not implement GitCode release publishing automation in CI~~ — **Updated 2026-06-13**: GitCode CI workflow (`.gitcode/workflows/release.yml`) auto-builds and commits artifacts to `dist/release/{tag}/`.
...
**Constraints** - Binary replacement supports Linux/macOS/Windows; Linux/macOS auto-restart via `syscall.Exec`; Windows replaces the binary but requires manual restart; replacement must write a complete temporary file before swapping paths and must rollback on install failure; archive downloads must reject responses larger than 200 MB and checksum downloads must reject responses larger than 1 MB.
...
**Outcomes** - `GET /api/update/check` returns version comparison info; `POST /api/update/apply` triggers download and apply; both are behind `authMiddlewareFunc`; when no updater is configured, endpoints return HTTP 503; in Docker, check remains available and apply returns a clear business error telling users to update the image.
...
**Edge Cases** - Updater not configured; Docker apply is disabled while check remains available; all sources fail; already up to date; download or verification fails; apply called with GET method.
...
**Constraints** - Failed update check must be silent (label stays as static version text, no error indicator); page-load update checks must be throttled to once per 24 hours per browser using local storage, and the attempt must be recorded before the network request so repeated failures do not retry within 24 hours; the label must not shift the header layout when transitioning between states; the dialog must show the confirm message about service interruption; the apply button must show a loading state during update.

**Edge Cases** - Version is `dev` (local build without ldflags) — label shows `dev` until a permitted update check reports a newer release; update check fails silently — label stays as static version text and page-load checks are not retried for 24 hours; apply fails with an error message; apply succeeds and server restarts (UI shows "restarting" message); user dismisses the dialog.
...
6. Call `checkUpdate()` on component mount only when the last page-load check is older than 24 hours (best-effort, silent on failure).
...
- [ ] Page-load update checks are skipped when the last attempt is within 24 hours.

### Task 6: Startup Auto-Check and Docker Detection

#### Requirements

**Objective** - Wire the updater into `main.go` with Docker environment detection and no server-side startup update check.

**Outcomes** - On startup, the updater is instantiated and injected into the admin server via `SetUpdater` in both bare-metal and Docker environments; in Docker, apply is disabled with a guidance message while checks remain available.

**Evidence** - Application starts successfully; Docker log output shows image-update guidance; admin update check works in both Docker and non-Docker environments; admin update apply is blocked in Docker.

**Constraints** - Server startup must not perform an outbound update check; update checks must happen through authenticated admin API requests; Docker must not perform in-place binary replacement.
...
5. Do not launch a startup goroutine for update checks.

#### Verification

- [ ] Non-Docker startup wires the updater without making an outbound update-check request.
- [ ] Docker startup logs guidance message and still creates an updater for checks.
- [ ] Admin update check returns 200 in Docker when release sources are reachable.
- [ ] Admin update apply returns a 200 business error in Docker, with `restarting:false`.
- [ ] Startup is not delayed by any update check.
...
- [ ] Docker detection keeps update checks available and disables only in-place self-update.
