...
- **I2 行内和**：四个 token 均非负，且从左到右累加的每个中间前缀（含最终和）均在
  int64 内。由于各项非负，这等价于行内最终和位于 `[0, MaxInt64]`。
- **I3 跨行和**：任意聚合分组中的 token 与 duration 均非负，且 `SUM()` 的每个中间
  累加前缀（而非仅最终结果）均在 int64 内；适用于 Summary 全局、Providers/Models
  每组及 Trends 每桶。

真实产品数据恒满足 I2/I3：单计数器是真实非负 token 计数（≤ ~1e9 量级）、行数 ≤ 数十万、
