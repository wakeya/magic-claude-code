...
- [ ] `POST /api/providers/export` 传有效 ID 返回含真实 token 的供应商。
- [ ] 未知 ID 静默跳过。
- [ ] 响应结构匹配导出文件格式（`version`、`exported_at`、`providers`）。
...
- [ ] `skip` 策略保留冲突供应商不变。
- [ ] `overwrite` 策略替换字段，保留 `created_at`。
- [ ] `duplicate` 策略以新 ID 追加。
- [ ] 无效供应商被跳过并报告到 `errors`。
- [ ] `version != 1` 返回明确错误。
