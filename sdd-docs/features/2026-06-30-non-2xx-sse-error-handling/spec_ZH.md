...
9. 详细错误日志只包含经过类型检查的安全请求字段：`model`、`stream`、数值生成参数，以及 `messages`、`tools` 或 `input` 的数量。携带提示词、身份信息、凭据和未知内容的字段必须省略。
...
5. 保持现有兼容性请求头摘要方式。请求参数必须使用显式类型白名单；不得记录原始系统提示词、metadata、消息/input 内容、工具 schema、凭据、授权值或未知扩展。
