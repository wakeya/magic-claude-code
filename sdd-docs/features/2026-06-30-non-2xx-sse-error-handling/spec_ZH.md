...
最后更新：2026-07-01
进度：后续任务已规划，3 / 4 已完成；任务 4 等待规格审阅
...
| 4 | 已规划 | 增加有界协议结构诊断 | `internal/proxy/request_diagnostics.go`、定向测试、Handler 日志断言 | RED 证明现有摘要缺少 stream 存在性/工具/schema 结构；GREEN 保持零敏感标记和有界输出 |
...
- [ ] 可以区分缺失、false、true 和类型错误的 `stream` 状态。
- [ ] 可以通过摘要比较工具组成变化，并显示已知 Web 工具。
- [ ] 统计兼容性相关 schema 特征，但不保留 schema 内容。
- [ ] 显示 message 结构，但不包含消息正文、tool input 或 tool result 内容。
- [ ] system、metadata、thinking、input 和未知字段只暴露形态/数量。
- [ ] 大集合的摘要大小保持有界。
- [ ] 现有响应、用量、心跳、修复器和安全行为保持不变。
- [ ] 日志可以区分 beta 存在和 URL 脱敏，同时不泄露其他 query 参数。
- [ ] 定向测试、完整代理测试、`make test` 和新的安全审查通过。
