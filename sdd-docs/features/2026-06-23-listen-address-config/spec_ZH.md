...
| 序号 | 状态 | 任务 | 产出 | 验证 |
| --- | --- | --- | --- | --- |
| 1 | 已完成 | Config 字段与默认值 | `internal/config/config.go`、`config_test.go` | 默认值与 normalize 单元测试 |
| 2 | 已完成 | 持久化层支持新字段 | `internal/config/sqlite_store.go`、`sqlite_store_test.go` | 新旧数据库迁移测试 |
| 3 | 已完成 | CLI flag + 环境变量覆盖 | `cmd/server/main.go` | flag 解析与 env 覆盖单元测试（`main_test.go`） |
| 4 | 已完成 | 启动接线，消除 `:443`/`:8442` 硬编码 | `cmd/server/main.go` | 启动监听地址与配置一致；横幅打印真实地址 |
| 5 | 已完成 | `/api/status` 暴露代理与 Admin 监听 | `internal/admin/handler.go`、`handler` 相关测试 | 状态接口字段断言测试 |
| 6 | 已完成 | 前端只读展示监听状态 + i18n | `internal/frontend/src/views/DashboardView.vue`、`useI18n.ts`、`useApi.ts` | 前端构建 + 组件断言测试 |
| 7 | 已完成 | CLI 帮助 i18n + 版本 flag | `cmd/server/main.go`、`internal/i18n/i18n.go` | `mcc -h` / `mcc -v` 中英文输出验证 |
...
5. `main.go` 启动代理与 Admin 时使用 `net.JoinHostPort(cfg.ProxyListenAddr, strconv.Itoa(cfg.ProxyPort))` 组装地址，消除硬编码 `:443` / `:8442`；Gateway 同样统一使用 `net.JoinHostPort`。
