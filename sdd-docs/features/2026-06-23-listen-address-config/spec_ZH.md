...
3. SQLite store 持久化 `proxy_listen_addr` / `admin_listen_addr`；读取旧数据库（无此列）时回退默认值，不报错。
...
5. SQLite 读取旧数据库（无 `proxy_listen_addr` / `admin_listen_addr` 列）时必须无缝回退默认值，不触发迁移错误。
...
**Objective（目标）** — 让新字段在 SQLite 存储中可读可写，且兼容旧数据库。

**Outcomes（成果）** — `sqlite_store.go` 写入/读取 `proxy_listen_addr` / `admin_listen_addr`；旧库（无此列）读取时回退默认值，不报错。

**Evidence（证据）** — 单元测试：新建库往返一致；旧库（模拟缺列）读取不报错且回退默认。

**Constraints（约束）** — 沿用现有 SQLite schema 迁移风格（项目已有"自动补列"先例）；不破坏既有字段读写。

**Edge Cases（边界）** — 列不存在；列存在但值为空串；列存在但端口为 0 或非数字。

**Verification（验证）** — `go test ./internal/config/`。

#### 计划

1. 在写入路径添加 `proxy_listen_addr` / `admin_listen_addr` 键值。
2. 在读取路径添加对应解析；空串/缺列 → 不赋值，交给 `NormalizeConfig` 填默认。
3. 端口字段沿用现有 `ProxyPort`/`AdminPort` 读写逻辑（已有）。
4. 测试模拟旧库（不写入新列）验证回退。
