  ...
  <div class="grid grid-cols-1 gap-4 xl:grid-cols-[320px_minmax(0,1fr)]">
    <aside class="rounded-lg border-2 border-border bg-white">
      ...
      <div class="max-h-[calc(100vh-220px)] overflow-y-auto p-3">
        <button
          :class="projectButtonClass(selectedProject === '')"
          @click="selectProject('')"
        >
          <span>{{ t('sessions.all_projects') }}</span>
          <span class="rounded-full bg-muted px-2 py-0.5 text-xs">{{ totalSessions }}</span>
        </button>

        <button
          v-for="project in projects"
          :key="project.path"
          :class="projectButtonClass(selectedProject === project.path)"
          @click="selectProject(project.path)"
        >
          <span class="min-w-0 truncate">{{ project.name }}</span>
          <span class="rounded-full bg-muted px-2 py-0.5 text-xs">{{ project.session_count }}</span>
        </button>
    ...
    <section class="min-w-0">
      ...
      <div v-else class="grid grid-cols-1 gap-4 2xl:grid-cols-[minmax(0,1fr)_260px]">
        ...
        <aside class="hidden 2xl:block">
          <div class="sticky top-4 rounded-lg border-2 border-border bg-muted p-3">
  ...
  setup(props) {
    ...
    return () =>
      h('div', { class: 'mb-4' }, [
        ...
        h('div', { class: 'flex items-start gap-2 rounded-lg bg-fg p-3 text-gray-100' }, [
          h('code', { class: 'min-w-0 flex-1 whitespace-pre-wrap break-all font-mono text-sm' }, props.command),
          h(
            'button',
            { class: 'rounded-md bg-white/10 p-2 hover:bg-white/20', onClick: copyCommand, title: copied.value ? t('sessions.copied') : t('sessions.copy') },
...
function projectButtonClass(active: boolean): string {
  return [
    'mb-2 flex w-full items-center justify-between gap-2 rounded-lg px-3 py-2.5 text-left text-sm font-semibold transition',
    active ? 'bg-primary text-white' : 'bg-white text-fg hover:bg-muted',
  ].join(' ')
}
